...
Float Path::miWeight(const Scene *scene, const Path &emitterSubpath,
            const Path &connectionSubpath, const Path &sensorSubpath,
        int s, int t, bool sampleDirect, bool lightImage) {
    int pathVertices = connectionSubpath.vertexCount();
    int k = s+t+pathVertices+1, n = k+1;

	// emitterSubpath: from 0 to s, both included.
	// sensorSubpath: from t to 0, both included
	// connectionSubpath: from 0 to pathVertices-1
    ...
    for (int i=0; i < pathVertices; i++) {
        const PathVertex *v = connectionSubpath.vertex(i);
        connectable[pos] = v->isConnectable();
        isNull[pos] = v->isNullInteraction() && !connectable[pos];
        pos++;
	}
    ...
    for (int i=t; i>=0; --i) {
        ...
        isNull[pos] = v->isNullInteraction() && !connectable[pos];
        if (pos >= n) {
        	fprintf(stdout, "s = %d t = %d pathVertices = %d n = %d pos = %d\n", s, t, pathVertices, n, pos); 
        	fflush(stdout); 
		}
        assert(pos < n);
        pos++;
    ...
    pdfImp[pos++] = vs->evalPdf(scene, vsPred, vt, EImportance, vsMeasure) * connectionSubpath.edge(0)->pdf[EImportance]; // * connectionEdge->pdf[EImportance];

	for (int i=0; i < pathVertices; i++) {
        pdfImp[pos++] = connectionSubpath.vertex(i)->pdf[EImportance]
            * connectionSubpath.edge(i+1)->pdf[EImportance];
    }
	...
	for (int i=0; i < pathVertices; i++) {
        pdfRad[pos++] = connectionSubpath.vertex(i)->pdf[ERadiance]
            * connectionSubpath.edge(i)->pdf[ERadiance];
    }

    pdfRad[pos++] = vt->evalPdf(scene, vtPred, vs, ERadiance, vtMeasure) * connectionSubpath.edge(pathVertices)->pdf[ERadiance]; // * connectionEdge->pdf[ERadiance];
    ...
    for (int i=1; i <= k-3; ++i) {
        ...
        const PathVertex *cur = i <= s ? emitterSubpath.vertex(i) : i <= s+pathVertices ? connectionSubpath.vertex(i-s-1) : sensorSubpath.vertex(k-i);
        const PathVertex *succ = i+1 <= s ? emitterSubpath.vertex(i+1) : i+1 <= s+pathVertices ? connectionSubpath.vertex(i-s) : sensorSubpath.vertex(k-i-1);
        const PathEdge *edge = i < s ? emitterSubpath.edge(i) : i < s+pathVertices ? connectionSubpath.edge(i) : sensorSubpath.edge(k-i-1);
        

        pdfImp[i+1] *= edge->length * edge->length / std::abs(
            (succ->isOnSurface() ? dot(edge->d, succ->getGeometricNormal()) : 1) *
            (cur->isOnSurface()  ? dot(edge->d, cur->getGeometricNormal())  : 1));
        if ((i > s) && (i <= s+pathVertices)) pdfImp[i+1] *= 1000; 
    }
    ...
    for (int i=k-1; i >= 3; --i) {
        ...
        const PathVertex *cur = i <= s ? emitterSubpath.vertex(i) : i <= s+pathVertices? connectionSubpath.vertex(i-s-1) : sensorSubpath.vertex(k-i);
        const PathVertex *succ = i-1 <= s ? emitterSubpath.vertex(i-1) : i-1 <= s+pathVertices? connectionSubpath.vertex(i-2-s) : sensorSubpath.vertex(k-i);
        const PathEdge *edge = i <= s ? emitterSubpath.edge(i-1) : i <= s+pathVertices? connectionSubpath.edge(i-2-s) : sensorSubpath.edge(k- i -1);

        pdfRad[i-1] *= edge->length * edge->length / std::abs(
            (succ->isOnSurface() ? dot(edge->d, succ->getGeometricNormal()) : 1) *
            (cur->isOnSurface()  ? dot(edge->d, cur->getGeometricNormal())  : 1));
        if ((i > s) && (i <= s+pathVertices)) pdfRad[i-1] *= 1000; 
    }
    ...
    for (int i=1; i <= k-3; ++i) {
        ...
        const PathVertex *before = i     <= s ? emitterSubpath.vertex(i) : i <= s+pathVertices ? connectionSubpath.vertex(i-s-1) : sensorSubpath.vertex(k-i);
        const PathVertex *after  = end+1 <= s ? emitterSubpath.vertex(end+1) : end+1 <= s+pathVertices ? connectionSubpath.vertex(end-s) : sensorSubpath.vertex(k-end-1);
    ...
    if (sampleDirect) {
        /* Direct connection probability of the emitter */
        const PathVertex *sample = s>0 ? emitterSubpath.vertex(1) : vt;
        const PathVertex *ref = emitterRefIndirection <= s ? emitterSubpath.vertex(emitterRefIndirection) 
            : emitterRefIndirection <= s+pathVertices? connectionSubpath.vertex(emitterRefIndirection-s-1) 
            : sensorSubpath.vertex(k-pathVertices-emitterRefIndirection);
        ...
        sample = t>0 ? sensorSubpath.vertex(1) : vs;
        ref = sensorRefIndirection <= s ? emitterSubpath.vertex(sensorRefIndirection)
            : sensorRefIndirection <= s+pathVertices? connectionSubpath.vertex(sensorRefIndirection-s-1) 
            : sensorSubpath.vertex(k-sensorRefIndirection);
        measure = sample->getAbstractEmitter()->getDirectMeasure();
    ...
    /* With all of the above information, the MI weight can now be computed.
       ...
       to the (s,t) strategy, which can be done using a linear sweep. For
       details, refer to the Veach thesis, p.306. */
    // Normally, no changes. Then add connectingPath
    for (int i=s+pathVertices+1; i<k; ++i) {
        double next = pdf * (double) pdfImp[i] / (double) pdfRad[i],
               value = next;
    ...
    /* As above, but now computing the pdf[i] for the connecting path */
	// HERE, we add connectingPath vertices.
	// 
